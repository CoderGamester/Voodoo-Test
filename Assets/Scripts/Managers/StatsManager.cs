...
public class StatsManager : SingletonMB<StatsManager>
{
	...
	/*public void AddGameResult(int _WinScore)
	{
		...
		for (int i = Constants.c_SavedGameCount - 1; i >= 0; --i)
		{
			...
			PlayerPrefs.SetInt (key, m_DataProvider.PlayerDataProvider.GetGameResult(i - 1));
		}
	...
	public float GetLevel()
	{
		...
		for (int i = 0; i < Constants.c_SavedGameCount; ++i)
			result += m_DataProvider.PlayerDataProvider.GetGameResult(i);
	...
	{
		int currentLevel = _LevelStart == -1 ? m_DataProvider.PlayerDataProvider.PlayerLevel.Value - 1 : _LevelStart;
