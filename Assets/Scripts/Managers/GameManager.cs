...
public class GameManager : SingletonMB<GameManager>
{
    ...
    void Awake()
    {
		...
		m_GameServices = MainInstaller.Resolve<IGameServices>();
    ...
    public void ChangePhase(GamePhase _GamePhase)
    {
        switch (_GamePhase)
        {
            ...
            case GamePhase.END:
                ...
                var playerDataProvider = m_DataProvider.PlayerDataProvider;
				...
				PreEndView.Instance.LaunchPreEnd(playerDataProvider.PlayerXP.Value, playerDataProvider.PlayerLevel.Value - 1);

				m_GameServices.CommandService.ExecuteCommand(new EndGameResultsCommand
                {
                    MatchScore = playerScore,
                    MatchRank = rankingScore
				});
                break;
        }
    ...
    {
        int favoriteSkin = Mathf.Min(m_DataProvider.PlayerDataProvider.FavoriteSkin.Value, m_Skins.Count - 1);
